...
2. Read the complete files under `agent-lifecycle/agent-definition/`. Derive cases from stated users, risks, success outcomes, metrics, and commercial value without inventing requirements. Where real or promoted transcripts exist, cluster them by intent and draft cases from actual failures rather than imagined ones.
